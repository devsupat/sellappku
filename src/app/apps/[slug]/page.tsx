...
function getDownloadInfo(type: string) {
 switch (type) {
 case 'playstore':
 return { label: 'Download dari Play Store', icon: Play, color: 'bg-green-600 hover };
 case 'drive':
 return { label: 'Download dari Google Drive', icon: Download, color: 'bg-blue-600 hover };
 case 'mediafire':
 return { label: 'Download dari Mediafire', icon: Download, color: 'bg-indigo-600 hover };
 default:
 return { label: 'Download APK', icon: Download, color: 'bg-violet-600 hover };
 }
}
...
export default async function AppDetailPage({ params }: PageProps) {
 ...
 <p className="font-medium text-gray-900 ">{app.size || '-'}</p>
 </div>
 <div>
 <p className="text-sm text-gray-500 ">Terakhir Update</p>
 <p className="font-medium text-gray-900 ">{app.updated_date || '-'}</p>
 </div>
 <div>
 <p className="text-sm text-gray-500 ">Downloads</p>
 <p className="font-medium text-gray-900 ">{app.downloads || '0'}</p>
 </div>
 </div>
 </div>
 </div>
